...
`just test-one`/`test-debug`/`test-many` resolve each simple class name to a fully-qualified source name before invoking Gradle. This rejects missing or ambiguous names independently—especially important for `test-many`, where one valid selector could otherwise hide a typo. Repeated `--tests` flags provide native OR semantics. Kotest 6 also supports direct Gradle wildcard filters such as `--tests '*PackageLayeringTest'`; nonexistent filters fail the build.

Use Gradle's `--tests` filtering, not Kotest-specific project properties. The recipes intentionally expose one supported filtering path with loud no-match behavior.
...
3. Local stress iterations use only `./gradlew -PforceTests :engine:testGate :engine:testIntegration`. Run 3-5 forced passes max, and stop on the first failure.
4. After the first repro, switch to targeted repeats: `./gradlew -PforceTests :engine:testIntegration --tests "fully.qualified.ClassName"` (fully qualified, no wildcard — see "Running tests" above for why). If the targeted class passes but the full graph flakes, suspect task/fork/shared-state interaction before changing game code.
