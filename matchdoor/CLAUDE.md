...
**Per-frame card view: `BoundCard`.** `SnapshotCapture.bindCards` pairs every `CardSnapshot` with its static `CardData` and pre-resolves the static-derived live questions: alt-cost ability rows (`altCosts: List<AltCostBinding>`), Mobilize cleanup grpId (`mobilizeCleanup`), parent linkage (`parentLinkage: ParentLinkage?`), and card-state designations (`designations: DesignationSet`). Mappers read `snap.boundCards[fid]?.X` instead of reaching back into `bridge.cardRepository`. Residual `cardRepository` consumers, by design: `SnapshotCapture` + `GrpIdResolver` (the binder), `BundleBuilder` (engine-state bind), `ActionPerformer` (action-execution time, no per-frame snap), `GameEventCollector` (event stream, no per-card snap), `SourceAbilityResolverFactory` (abstraction not card-bound). Name-keyed lookups (`TargetingHandler` modal options, `StateMapper.buildTargetSpecAnnotations`) also stay residual — BoundCard is keyed on `ForgeCardId`, not card name. `GsmSnapshot.objects` is now a derived view of `boundCards.mapValues { it.snapshot }`; `boundCards` is the single source of truth and `equals`/`hashCode` ignore the derived projection.
