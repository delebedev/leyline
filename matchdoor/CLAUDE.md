...
- **44 override families, pinned by `PlayerControllerStructureTest`.** Adding or removing one requires updating the test and the table below in the same commit.
...
All 44 override families, by concern. "Bridge" column names the primary mechanism each uses.
