...
class AnnotationOrderEnforcerTest :
    FunSpec({
        ...
        // ===== Damage-before-death: DamageDealt must precede OIC/ZT for dying creature =====

        test("damage-before-death: DamageDealt before ObjectIdChanged for same creature") {
            val dd = AnnotationBuilder.damageDealt(sourceInstanceId = 100, targetId = 500, amount = 3)
            val oic = AnnotationBuilder.objectIdChanged(origId = 500, newId = 600)

            // Wrong order: OIC before DD
            val result = AnnotationOrderEnforcer.enforce(listOf(oic, dd))
            result.map { it.typeList.first() } shouldBe listOf(
                AnnotationType.DamageDealt_af5a,
                AnnotationType.ObjectIdChanged,
            )
        ...
        test("damage-before-death: DamageDealt before ZoneTransfer for same creature") {
            val dd = AnnotationBuilder.damageDealt(sourceInstanceId = 100, targetId = 500, amount = 3)
            val zt = AnnotationBuilder.zoneTransfer(
                instanceId = 500,
                srcZoneId = 28,
            ...
            // Wrong order: ZT before DD
            val result = AnnotationOrderEnforcer.enforce(listOf(zt, dd))
            result.map { it.typeList.first() } shouldBe listOf(
                AnnotationType.DamageDealt_af5a,
                AnnotationType.ZoneTransfer_af5a,
            )
        ...
        test("damage-before-death: full combat kill — DD then OIC then ZT") {
            val dd = AnnotationBuilder.damageDealt(sourceInstanceId = 100, targetId = 500, amount = 5)
            val oic = AnnotationBuilder.objectIdChanged(origId = 500, newId = 600)
            val zt = AnnotationBuilder.zoneTransfer(
                instanceId = 600,
                srcZoneId = 28,
            ...
            // Wrong order: OIC, ZT, DD (transfers built before combat)
            val result = AnnotationOrderEnforcer.enforce(listOf(oic, zt, dd))
            result.map { it.typeList.first() } shouldBe listOf(
                AnnotationType.DamageDealt_af5a,
                AnnotationType.ObjectIdChanged,
                AnnotationType.ZoneTransfer_af5a,
        ...
        test("damage-before-death: no-op when DamageDealt and ZT affect different cards") {
            val dd = AnnotationBuilder.damageDealt(sourceInstanceId = 100, targetId = 500, amount = 3)
            val oic = AnnotationBuilder.objectIdChanged(origId = 700, newId = 800)
            val zt = AnnotationBuilder.zoneTransfer(
                instanceId = 800,
                srcZoneId = 28,
                destZoneId = 33,
            ...
            // Different cards — no damage-before-death constraint (Rule 1 OIC→ZT still applies)
            val input = listOf(oic, zt, dd)
            val result = AnnotationOrderEnforcer.enforce(input)
            result shouldBe input
        }

        test("damage-before-death: multiple creatures take lethal") {
            val dd1 = AnnotationBuilder.damageDealt(sourceInstanceId = 100, targetId = 500, amount = 3)
            val dd2 = AnnotationBuilder.damageDealt(sourceInstanceId = 200, targetId = 600, amount = 4)
            val oic1 = AnnotationBuilder.objectIdChanged(origId = 500, newId = 700)
            val oic2 = AnnotationBuilder.objectIdChanged(origId = 600, newId = 800)
            val zt1 = AnnotationBuilder.zoneTransfer(
                instanceId = 700,
                srcZoneId = 28,
                destZoneId = 33,
            ...
            // Wrong: all transfers before all damage
            val result = AnnotationOrderEnforcer.enforce(listOf(oic1, zt1, oic2, zt2, dd1, dd2))

            // DD1 before OIC1/ZT1, DD2 before OIC2/ZT2
            val dd1Idx = result.indexOfFirst {
                it.typeList.first() == AnnotationType.DamageDealt_af5a && it.getAffectedIds(0) == 500
            }
            val oic1Idx = result.indexOfFirst {
                it.typeList.first() == AnnotationType.ObjectIdChanged && it.affectedIdsList.contains(500)
            }
            val zt1Idx = result.indexOfFirst {
                it.typeList.first() == AnnotationType.ZoneTransfer_af5a && it.affectedIdsList.contains(700)
            }
            val dd2Idx = result.indexOfFirst {
                it.typeList.first() == AnnotationType.DamageDealt_af5a && it.getAffectedIds(0) == 600
            }
            val oic2Idx = result.indexOfFirst {
                it.typeList.first() == AnnotationType.ObjectIdChanged && it.affectedIdsList.contains(600)
            }
            val zt2Idx = result.indexOfFirst {
                it.typeList.first() == AnnotationType.ZoneTransfer_af5a && it.affectedIdsList.contains(800)
            }

            (dd1Idx < oic1Idx) shouldBe true
            (oic1Idx < zt1Idx) shouldBe true
            (dd2Idx < oic2Idx) shouldBe true
            (oic2Idx < zt2Idx) shouldBe true
        }
