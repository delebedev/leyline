...
class ZoneTransferTest :
    SubsystemTest({
        ...
        test("Battlefield → Graveyard (Destroy)") {
            ...
            val (gsm, newId) =
                transferCard(b, game, counter, "Grizzly Bears") { card, g ->
                    destroy(card, g)
                }
        ...
        test("Library → Hand (Draw)") {
            ...
            val (gsm, newId) =
                transferCard(b, game, counter, "Forest") { _, g ->
        ...
        test("Library → Graveyard (Mill)") {
            ...
            val (gsm, newId) =
                transferCard(b, game, counter, "Forest") { card, g ->
                    g.action.moveToGraveyard(card, null)
                }
        ...
        test("Hand → Graveyard (Discard)") {
            ...
            val (gsm, newId) =
                transferCard(b, game, counter, "Lightning Bolt") { card, g ->
        ...
        test("SBA: zero toughness → Destroy") {
            ...
            val (gsm, newId) =
                transferCard(b, game, counter, "Grizzly Bears", checkSba = true) { card, _ ->
