...
class StructuralFingerprintTest :
    FunSpec({
        ...
        test("roundTripJsonSerialization") {
            ...
            val json = ReferenceSequence.toJson(sequence)
            val parsed = ReferenceSequence.fromJson(json)
            parsed shouldBe sequence
        }

        test("loadReferenceFromFile") {
            val fp = StructuralFingerprint(
            ...
            val json = ReferenceSequence.toJson(listOf(fp))
            val tempFile = java.io.File.createTempFile("reference-test", ".json")
            tempFile.writeText(json)
            tempFile.deleteOnExit()
            val loaded = ReferenceSequence.fromFile(tempFile)
