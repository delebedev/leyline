...
object PlayerMapper {
    ...
    /** Inactivity timers — compatibility flow uses 2 per game state (one per seat). */
    fun buildTimers(): List<TimerInfo> =
        listOf(
            TimerInfo
                .newBuilder()
                .setTimerId(1)
    ...
    fun mapPhase(phase: PhaseType?): Phase =
        when (phase) {
    ...
    fun mapStep(phase: PhaseType?): Step =
        when (phase) {
            null -> Step.None_a2cb
