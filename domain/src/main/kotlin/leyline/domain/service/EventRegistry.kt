 ...
 *
 * ### Key fields
 * - `EventBladeBehavior: "Queue"` → Find Match only. Client expects prio -1 for all queue events.
 ...
 *   the local AI challenge event in Events (prio 92), not from the AiBotMatches array (prio -1).
 *
...
object EventRegistry {
    ...
    /** Queue config used for lobby/bootstrap flows. */
    val queues: List<QueueEntry> = loadResource("/fd-bootstrap/queues.json")

    val events: List<EventDef> = loadResource("/fd-bootstrap/events.json")
