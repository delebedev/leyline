...
graph TB
    subgraph "Client (Unity)"
        CLIENT["Game Client<br/><small>Unity · C# · protobuf</small>"]
    end
    ...
    subgraph "leyline (Kotlin)"
        ...
        DEBUG["debug/<br/><small>DebugServer · DebugCollector<br/>DebugEventBus</small>"]
    end
    ...
    CLIENT -- "6-byte frame + protobuf<br/>TLS TCP :30003" --> SRV
    SRV --> GAME_PKG
...
```
┌─────────────────────────────────────────┐
│  Client Wire Frame (6 bytes + payload)  │
├──────┬──────┬───────────────────────────┤
...
leyline/src/main/kotlin/leyline/              65 files, ~14.5K LOC
  ...
  ├── debug/          (3 files, ~300 LOC)     ← Debug panel: DebugServer, DebugCollector, DebugSinkAdapter
...
leyline/src/test/kotlin/                       63 files, ~13.4K LOC
  ...
  ├── conformance/    (48 files)              ← Integration + conformance tests,
  │                                              MatchFlowHarness (test infra)
...
**Event-driven annotations.** The `GameEventCollector → GameEvent → TransferCategoryResolver.categoryFromEvents()` chain decouples Forge's Guava EventBus from proto construction. Adding a new annotation type is a 5-step cookbook recipe touching known files.

**Conformance infrastructure.** `StructuralFingerprint` / `StructuralDiff` / golden files / `ValidatingMessageSink` / `MatchFlowHarness` — the testing infra is more sophisticated than most game servers. Conformance tests are the gold standard.

**Observability.** Debug server on :8090 with SSE, state timeline, instance history, priority trace, session introspection. This is production-grade observability for a dev project.
