...
3. **L1 life gain replacement not wired.** The `R:Event$ GainLife` replacement (grpId 133622) must intercept life gain events and add +1. The wire shape for replacement effects is documented at `docs/annotation-field-notes.md` (ReplacementEffect pAnn observed for Reduce to Ashes). Expected: `ReplacementEffect` transient annotation + the modified `ModifiedLife` value. Exact sequencing unconfirmed for this card.
