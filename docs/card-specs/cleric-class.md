...
5. **L3 trigger (reanimate from GY) not wired and unobserved.** The `T:Mode$ ClassLevelGained` trigger (grpId 146501) fires when the Class becomes L3. Requires `SelectTargetsReq` for target GY creature, ZoneTransfer GY→BF (category unknown — likely "Return" or "Reanimate"), then a life gain equal to toughness. No data point exists in this project's games.
