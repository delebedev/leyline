...
- `MatchCutCoordinator` now owns journal close, immutable cut materialization,
  ...
  and Search windows, iterative Convoke/Improvise/Waterbend payments, and Optional, Numeric, and Damage blocking interactions. Safe direct
...
- `PhaseHandler` provides broad step completion plus narrow UI-neutral combat
  hooks. Grouping, modal, select-N, one-shot payment, ordering, and automatic
