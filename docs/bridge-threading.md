...
Legend Rule, library putback, Manifest Dread, discard, resolution sacrifice, Suspect, and Mutate top/bottom callbacks freeze their route kind, source, cardinality, default, candidates, and exact card handles on the engine thread. Manifest Dread also projects the selected library candidates privately to the chooser in the same committed state-and-`SelectNReq` cut. Correlated `SelectNResp` values resolve through the retained instance-id table; Legend Rule, library putback, and Manifest Dread accept only that response family, while the other card-select routes retain their compatible `EffectCostResp` shape. Choice-result facts are staged before the exact engine wait is released, while Legend Rule records each unchosen exact handle after the wait returns and before state-based actions continue. Timeout retires the window and returns the configured default handle, while stale or invalid responses have no side effects.
