...
- **IMPORTANT: Diagnose with println first, reason about code second.** When something doesn't fire or behaves unexpectedly, add a 1-line `println` diagnostic and run `just test-debug` BEFORE tracing code paths. A 5-minute `println("stack size=" + game.stack.size())` saves 30+ minutes of reasoning about timing, threading, and call chains that turn out to be irrelevant. The diagnostic tells you WHAT is wrong; then you reason about WHY.
...
- **Annotation ordering: OIC+ZT is an atomic pair for the client.** Never insert DamageDealt between ObjectIdChanged and ZoneTransfer — the client's animation pipeline expects them together. AnnotationOrderEnforcer cannot fix damage-before-death ordering because Rule 1 (OIC before refs) forces OIC→DD, and damage-before-death forces DD→ZT, splitting the OIC+ZT pair. Real server emits DD first, then OIC+ZT. Fix requires pipeline stage reorder in StateMapper, not post-hoc enforcer. (PR #335 attempted enforcer approach, caused creatures to visually stick on battlefield.)
- **scry-ts sequences conflates seat perspectives.** Player.log contains both SendAndRecord (acting player, no echo) and SendHiFi (opponent view, with echo). scry sequences reports combined GSM counts without filtering — "real=6 GSMs" includes opponent-only echoes. Always check updateType when interpreting scry output. Bare echoes only appear after SendHiFi, never SendAndRecord.
