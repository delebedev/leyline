...
| Platform | File | Notes |
|----------|------|-------|
| macOS (Apple Silicon) | `Leyline_vX.Y.Z_aarch64.dmg` | Right-click → Open on first launch |
...
2. Right-click the app → Open (first launch only — bypasses Gatekeeper for unsigned apps)
3. Click **Play** — the app finds Arena, configures it, and starts the server

On first launch, macOS will ask for your password to trust the Leyline TLS certificate in your login keychain. This is required for Arena to connect securely.
...
The game engine loads ~32,000 card scripts on first start, which takes 30-60 seconds depending on your machine. This is normal. Subsequent starts are faster if the server stays cached in memory.
</details>
