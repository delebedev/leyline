 ...
 * resolving card names → Arena grpIds via [CardDb.lookupByName].
 * Cards not found in the Arena DB get a fallback grpId of 0 (face-down).
 */
class DeckProvider {
    ...
    companion object {
        ...
        // Known Arena grpIds for precon lands (FDN set)
    ...
    /** Resolve a card name to its Arena grpId, with fallback. */
    private fun resolveGrpId(cardName: String): Int {
        val grpId = CardDb.lookupByName(cardName)
        if (grpId == null) {
            log.warn("Card '{}' not found in Arena DB, using fallback grpId={}", cardName, FALLBACK_GRPID)
