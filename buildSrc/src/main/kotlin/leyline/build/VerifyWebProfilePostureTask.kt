...
abstract class VerifyWebProfilePostureTask : DefaultTask() {
    ...
    fun verify() {
        ...
        val forbiddenNames = listOf("frontdoor", "account")
        val forbiddenEntries = listOf("leyline/frontdoor/", "leyline/account/")
        val forbiddenText = "30010"

        val leakedNames = files.filter { file -> forbiddenNames.any { file.name.contains(it, ignoreCase = true) } }
        ...
        check(leakedPort.isEmpty()) { "web profile includes local Front Door port literal: ${leakedPort.joinToString { it.name }}" }
    }
}
