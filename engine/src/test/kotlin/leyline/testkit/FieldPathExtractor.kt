...
object FieldPathExtractor {
    ...
    fun formatDiff(
        ...
        val missing = reference - ours
        val extra = ours - reference
        ...
        if (newGaps.isNotEmpty()) {
            sb.appendLine("NEW GAPS (reference has, we don't, not in expectedMissing):")
        ...
        if (newExtras.isNotEmpty()) {
            sb.appendLine("NEW EXTRAS (we produce, reference doesn't, not in expectedExtra):")
