    ...
    FunSpec({
        ...
        test("the headless seam stays small and value-oriented") {
            HeadlessMatch::class.java.declaredMethods
                ...
                .toSet() shouldBe
                setOf("start", "submit", "advance", "observe", "checkpoint", "messagesSince", "diagnostics", "close")

            MatchFlowHarness::class.java.getDeclaredField("bridge").type.name shouldBe "leyline.game.state.GameBridge"
        ...
        test("normal startup and semantic intent produce an immutable observation") {
            ...
            try {
                ...
                assertSoftly {
                    action.client.objects.isNotEmpty() shouldBe true
        ...
        test("puzzle startup, advancement, diagnostics, and cleanup cross the seam") {
            ...
            val result = try {
                ...
                text.contains("session diagnostics") shouldBe true
                match.close()
            }
